  ...
  public function file(id:String, theDir:String = ''):String
  {
    ...
    for (modDir in dirs)
    {
      ...
      if (!resultLocalized)
      {
        ...
        if (fileSystem.exists(filePath)) result = filePath;
      }
    }
    return result;
  }
  ...
  private function _checkExists(id:String):Bool
  {
    ...
    for (d in dirs)
    {
      ...
      if (localeAssetPrefix != null)
      {
        ...
        if (fileSystem.exists(localePath)) return true;
      }
      // Else, FireTongue not enabled.
      #end
  ...
  private function initMod(d:String):Void
  {
    ...
    for (f in all)
    {
      ...
      if (assetType == FONT)
      {
        ...
        @:privateAccess if (!openfl.text.Font.__fontByName.exists(font.name)) openfl.text.Font.registerFont(font);
      }
      #end
    }
